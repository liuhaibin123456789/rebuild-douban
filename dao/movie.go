...
func SelectMvById(id int) (model.Movie, error) {
	...
	//return mv, err
	if err := tool.GDb.Model(&mv).Where("id=?", id).Find(&mv).Error; err != nil {
...
func SelectMvsByHot() ([]model.OfMovie, error) {
	...
	//}
	//return mvs, err
	if err := tool.GDb.Model(&model.Movie{}).Where("form=? and DATEDIFF(NOW(),release_time)<30", "电影").Find(&mvs).Error; err != nil {
	...
	for _, mv := range mvs {
		...
		ofMv.Score = (mv.OneStarNum + mv.TwoStarNum*2 + mv.ThreeStarNum*3 + 4*mv.FourStarNum + 5*mv.FiveStarNum) / (mv.OneStarNum + mv.TwoStarNum + mv.ThreeStarNum + mv.FourStarNum + mv.FiveStarNum)
...
func SelectMvsByFuture() ([]model.OfMovie, error) {
	...
	//}
	//return mvs, err
	if err := tool.GDb.Model(&model.Movie{}).Where("form=? and DATEDIFF(NOW(),release_time)<0", "电影").Find(&mvs).Error; err != nil {
		return []model.OfMovie{}, err
	}
	//数据迁移
	for _, mv := range mvs {
...
func InsertUDiscuss(ud model.UnderDiscuss) error {
	...
	var mvName string
	//查询不使用事务，检查mv_id是否存在
	if err := tool.GDb.Model(&model.Movie{}).Select("name").Where("id=?", ud.FromMvId).Find(&mvName).Error; err != nil {
		return err
	}
	//检查discuss_id是否存在
	if err := tool.GDb.Model(&model.MvDiscuss{}).Where("id=?", ud.Id).Find(&md).Error; err != nil {
		return err
	}
	//更新最后一条评论的标记
	if err := tx.Model(&model.MvUnderDiscuss{}).Where("id=?", ud.Id).Update("label", "1").Error; err != nil {
		tx.Rollback()
		return err
	}
	//插入评论
	md1 := model.MvUnderDiscuss{}
	md1.Id = ud.Id
	md1.FromMvId = ud.FromMvId
	md1.FromPhone = ud.FromPhone
	md1.UsedNum = ud.UsedNum
	md1.Content = ud.Content
	md1.Label = ud.Label
	md1.DateTime = ud.DateTime
	md1.ToPhone = ud.ToPhone
	if err := tx.Model(&model.MvDiscuss{}).Create(&md1).Error; err != nil {
		tx.Rollback()
		return err
	}
...
func DelDisOfMv(dId int, phone string) error {
	...
	//检查用户是否发表该影评
	var p string
	if err := tool.GDb.Model(&model.MvDiscuss{}).Select("from_phone").Where("id=?", dId).Find(&p).Error; err != nil {
		return errors.New("不能删除")
	}
	//删除讨论
	if err := tx.Where("id=?", dId).Delete(&model.MvDiscuss{}).Error; err != nil {
		tx.Rollback()
		return err
	}
	//删除讨论下的回复
	if err := tx.Where("id=?", dId).Delete(&model.MvUnderDiscuss{}).Error; err != nil {
		tx.Rollback()
		return err
	}
	return tx.Commit().Error
...
func DelUDisOfMv(dId int, phone string) error {
	...
	//return err
	return tool.GDb.Model(&model.MvUnderDiscuss{}).Where("from_phone=?", phone).Where("id=?", dId).Update("content", "【已删除】").Error
}
...
func InsertDiscuss(discuss model.Discuss) error {
	...
	//return err
	var name string
	if err := tool.GDb.Model(&model.Movie{}).Select("mv_name").Where("id=?", discuss.FromMvId).Find(&name).Error; err != nil {
		return err
	}
	var md = model.MvDiscuss{}
	md.FromPhone = discuss.FromPhone
	md.FromMvId = discuss.FromMvId
	md.Title = discuss.Title
	md.Content = discuss.Content
	md.DateTime = discuss.DateTime
	if err := tool.GDb.Model(&model.MvDiscuss{}).Create(&md).Error; err != nil {
		return err
	}
	return nil
}
